import { check_diagonal_tl_br, check_diagonal_tr_bl, check_horizontal, check_vertical } from "./matchUtils";
...
export const puzzle_01 = (input: string): number => {
  ...
  for (let y = 0; y <= y_axis_length - buffer; y++) {
    ...
    for (let x = 0; x <= x_axis_length - buffer; x++) {
      ...
      const isLastRow = y === y_axis_length - buffer;
      const isLastColumn = x === x_axis_length - buffer;



      const quadrantToCheck = input_matrix
        .slice(y, y + buffer)
        .map((line) => line.slice(x, x + buffer)) as unknown as Quadrant;

      //console.log(`\n\nChecking new Quadrant: (x:${x}, y:${y})`)
      //console.log(quadrantToCheck.map(line => line.join(" ")).join("\n"))

      const foundXMAS =
        check_horizontal(quadrantToCheck, isLastRow) +
        check_vertical(quadrantToCheck, isLastColumn) +
